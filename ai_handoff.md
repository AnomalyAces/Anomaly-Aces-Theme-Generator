...
* **Memory Cache Invalidation (`load_stylebox_uncached`)**: Implemented `ResourceLoader.CACHE_MODE_REPLACE` reloading for both `StyleBoxTexture` resources and their underlying `Texture2D` image assets in `ThemePreview.gd`, `ThemeBuilder.gd`, `StyleboxBuilder.gd`, and `ThemeExporter.gd`. This forces Godot to invalidate in-memory texture caches upon re-exporting Figma SVGs/images without requiring a Godot project restart.
* **Knob & Slider Drop Shadow Effect Parsing**: Removed `"knob"` and `"slider"` from texture-only exclusions in `StyleboxBuilder.gd`, enabling circular knob components with `DROP_SHADOW` metadata (e.g. `Red_Slider_Knob_Pressed.svg`) to build glowing `StyleBoxFlat` resources with Figma shadow colors, solid background fills, and circular corner radii.
